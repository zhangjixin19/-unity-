...
{
    [AddComponentMenu("Layout/Grid Layout Group Scalable", 153)]
    public class GridLayoutGroupScalable : LayoutGroup
    {
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                // SetChildAlongAxis(rectChildren[i], 1, startOffset.y + (cellSize[1] + spacing[1]) * positionY, cellSize[1]);原代码

                //下面为更改后的代码
                
                float realsizeY = ((height - ( (actualCellCountY - 1))) / actualCellCountY); 
                float realsizeX = realsizeY/2;

                SetChildAlongAxis(rectChildren[i], 0, startOffset.x + (realsizeX + spacing[0]) * positionX, realsizeX);
                SetChildAlongAxis(rectChildren[i], 1, startOffset.y + ( spacing[1]) * positionY, realsizeY);//改之后的代码
                //下面为网上更改的代码示例
                /*
